...
public:
    int run(int argc, char **argv) {
        ...
        real *train_sum_loss = callocator<real>().allocate(option.num_threads_);
        real *train_sum_reg = callocator<real>().allocate(option.num_threads_);
        ...
        std::fill_n(train_sum_loss, option.num_threads_, 0.0);
        std::fill_n(train_sum_reg, option.num_threads_, 0.0);
        ...
        std::function<void(int)> updater_func = [&](int thread_index) -> void {
            ...
            cout << boost::format("rank: %d, thread_index: %d, part_index: %d") % rank % thread_index % part_index
                 << endl;
...
//                                                 (1.0 + decay_rate * pow(col_update_counts[col_index] + 1, 1.5));
                ...
                train_sum_loss[thread_index] = train_loss;
                train_sum_reg[thread_index] = train_reg;
            ...
            if (option.output_path_.length() > 0) {
                ...
                ofstream::openmode mode = (part_index % option.num_threads_ == 0) ? ofstream::out : (ofstream::out |
                                                                                                     ofstream::app);
        ...
        for (unsigned int main_timeout_iter = 0; main_timeout_iter < option.timeouts_.size(); main_timeout_iter++) {
            ...
            int machine_train_count_error = std::accumulate(train_count_errors,
                                                            train_count_errors + option.num_threads_, 0);
            int machine_test_count_error = std::accumulate(test_count_errors, test_count_errors + option.num_threads_,
                                                           0);
            real machine_train_sum_error = std::accumulate(train_sum_errors, train_sum_errors + option.num_threads_,
                                                           0.0);
            real machine_test_sum_error = std::accumulate(test_sum_errors, test_sum_errors + option.num_threads_, 0.0);

            real machine_train_sum_loss = std::accumulate(train_sum_loss, train_sum_loss + option.num_threads_, 0.0);
            ...
            real global_train_sum_loss = 0.0;
            MPI_Allreduce(&machine_train_sum_loss, &global_train_sum_loss, 1, MPI_DOUBLE,
                          MPI_SUM, MPI_COMM_WORLD);
            real global_test_sum_loss = 0.0;
            MPI_Allreduce(&machine_test_sum_loss, &global_test_sum_loss, 1, MPI_DOUBLE,
                          MPI_SUM, MPI_COMM_WORLD);
        ...
        callocator<real>().deallocate(train_sum_loss, option.num_threads_);
        callocator<real>().deallocate(train_sum_reg, option.num_threads_);
