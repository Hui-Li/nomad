...
int main(int argc, char **argv) {
    ...
    for (unsigned int i = 0; i < paths.size(); i++) {
        ...
        if (i == 0) {
            ...
            if (nrows != this_nrows) {
                cerr << "number of rows is different from that of previous file: " << nrows << " vs. " << this_nrows
                     << endl;
                return 1;
            }
            if (ncols != this_ncols) {
                cerr << "number of columns is different from that of previous file: " << nrows << " vs. " << this_nrows
                     << endl;
                return 1;
            }
        }
    ...
    row_new2old.reserve(nrows);
    col_new2old.reserve(ncols);
    ...
    for (unsigned int i = 0; i < paths.size(); i++) {
        ...
        for (int row_index = 0; row_index < nrows; row_index++) {
            if (row_index % 100000 == 0) {
                cout << "col_ind progress: " << row_index << " / " << nrows << " (" << (row_index * 100.0 / nrows)
                     << "%)" << endl;
            }
            int old_row_index = row_new2old[row_index];
        ...
        for (int row_index = 0; row_index < nrows; row_index++) {
            if (row_index % 100000 == 0) {
                cout << "values progress: " << row_index << " / " << nrows << " (" << (row_index * 100.0 / nrows)
                     << "%)" << endl;
            }
            int old_row_index = row_new2old[row_index];
            set<pair<int, double> > coords;
